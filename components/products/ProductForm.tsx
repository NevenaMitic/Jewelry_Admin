...
const formSchema = z.object({
  ...
  gemstone: z.string(),
  diamondWeight: z.coerce.number().min(0.1).max(100)
});

interface ProductFormProps {
  initialData?: ProductType | null; // Početni podaci za uređivanje proizvoda (ako postoji)
}
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      type="number"
                      placeholder="Weight Of The Diamond"
                      {...field}
                      onKeyDown={handleKeyPress}
                    />
                  </FormControl>
          ...
          <div className="flex gap-10">
            <Button type="submit" className="bg-beige-2 text-white">
              Save
            </Button>
            <Button
              type="button"
              onClick={() => router.push("/products")}
              className="bg-beige-2 text-white"
            >
              Discard
            </Button>
          </div>
